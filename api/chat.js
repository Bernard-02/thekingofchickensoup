...
- 只回傳英文翻譯，不要加任何其他文字或格式`;
    ...
    // 以「中」為 base，上下兩級拉出明顯差距（視覺上必須一眼看得出長短不同）
    const lengthDescByLevel = {
        0: `**極短**：4-8 個中文字（含標點）。像一個標語、一句口號，甩出就走。
範例：「別想了，去做。」（6 字）
範例：「累了就停。」（5 字）`,
        25: `**短（預設）**：16-22 個中文字（含標點）。一句話講完，不要拖。
範例：「硬撐不會讓你變強，只會讓你撐久。」（16 字）
範例：「累了就休息，別跟自己耗。明天還要走。」（18 字）`,
        50: `**中（預設）**：25-38 個中文字（含標點）。一到兩句，把話說清楚。
範例：「你不是走不動了，是腳沒地方放。找個地方坐下來再說吧。」（27 字）`,
        75: `**長**：55-80 個中文字（含標點）。兩到三句，展開得有層次。比「中」**至少多一倍的字**。
範例：「你一直在跑，但沒人規定每一步都要是對的。錯了也算走過，只是還沒停下來看而已。該停的時候停。」（44 字為低標，可再長到 70-80）`,
        100: `**很長**：95-130 個中文字（含標點）。一整段話，完整把心裡的意思講透。比「長」**再多一倍**。
範例：「你不是不夠努力，是太會責怪自己了。真的，有些日子就是這樣——不管做了什麼，感覺都在扣分。但那不是真的，那是你自己給自己的壓力。放一放，明天再試。」（78 字為低標，可再長到 100-130）`
    };
...
⚠️ **寫完後請按以下步驟自我檢查**：
1. 每寫完一句，在心裡數字數（一個中文字算 1，標點也算 1）
2. 如果不在上面的範圍內，**直接重寫這句**，不要將就
3. 5 個版本的字數應該**差不多**（都在同一範圍）

5 句都要符合這個長度範圍。做不到就重寫到做到為止。
...
- 要像說話一樣自然，不要太文藝
- ⚠ **絕對不要出現「我」這個代名詞**。你寫的雞湯是給觀眾看的觀察與提醒，不是在講自己。語句要是「你 / 他 / 人 / 事 / 一般陳述」，不要用「我覺得 / 我聽到 / 我想 / 我認為」這種第一人稱。寫完掃一遍，有「我」就重寫。
...
{
  "valid": true/false,
  "reasoning": "說明為什麼把這句話給這個觀眾。60-90 個中文字。結構：先一句話點出觀眾目前的狀態（直接敘述，不用比喻），再一句話講這狀態常見的問題或傾向，最後一句話解釋這句雞湯為什麼對上他。絕對不要用比喻（不要出現『沙子、蠟燭、水、影子、火』這類意象詞）、不要籤文、不要句尾用句號以外的符號營造詩意。寫法像朋友幫你解讀的說明，要讓觀眾讀完能清楚知道『喔，我被看見了、而且這句話是因為這個原因給我的』。範例：『你現在還在硬撐，嘴上說沒事但身體已經在抗議了。這種時候多數人會騙自己再一下就好，結果越撐越爛。所以給你這句——不是勸你放棄，是提醒你電量不是無限的。』",
